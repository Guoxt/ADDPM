...
class GaussianDiffusion:
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        #print(x_t.shape, t.shape, eps.shape)
    ...
    def p_sample_loop_progressive(
        ...
        #print('@@@',org_MRI.shape) #@@@ torch.Size([1, 4, 128, 192])
        if progress:
            ...
            ###生成第pre_t的预测
            ###
            img = torch.tensor(pre_seg).cuda()
            #print('@@@',img.shape)
            ###
            ###
            ###
            for i in indices[-pre_t:]:
                #print(i)
...
#                    import cv2
#                    temp = img.detach().cpu().numpy()*255
#                    cv2.imwrite('userhome/GUOXUTAO/2022_02/04/Diffusion-based-Segmentation00/results/Figure/img_'+str(i)+'.png',temp[0,0,...])
#                     import nibabel as nib 
#                     new_image = nib.Nifti1Image(temp[:,0,:,:], np.eye(4)) 
#                     nib.save(new_image, 'userhome/GUOXUTAO/2022_02/04/Diffusion-based-Segmentation00/results/Figure/img_'+str(i)+'.nii.gz') 
        
                with th.no_grad():
                    if img.shape != (1, 5, 128, 192):
                        #print('img.shape != (1, 5, 128, 192)!!!')
                    ...
                    yield out
                    img = out["sample"] # 重要注释：该行得到的img就是 b*1*W*H，其中1表示重建的单通道预测结果
                    #print(img.shape)
