...
def main():
    ...
    for epoch in range(args.max_epoch):
        ...
        for ii,(data,label) in tqdm(enumerate(train_dataloader),total=len(train_data)):
            ...
            ###
            noise = th.randn_like(data[:,0:1,:,:])
            noise_seg = diffusion.q_sample(label, th.tensor([args.T]* data.shape[0]), noise=noise)     #add noise to the segmentation channel
            
            # train model 
            input = Variable(noise_seg.float())
            labell = label.long()
